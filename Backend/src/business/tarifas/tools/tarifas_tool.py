...
from ..sources.tarifas_source import tarifas_sql
...
class ToolTarifas():
    ...
    def calcular_tarifas(self, valoresSUI, ipcMesAnterior, ipcMesConsulta):
        tarifas = pd.DataFrame(valoresSUI, dtype='float64')

        # print('-----------------------------')
        # print('TARIFAS >> ', tarifas.dtypes)
        # print('-----------------------------')

        # tarifas['juank'] = 123456

        #Consutla MongoDB GESTOR IDANE (trae solo IPC - mes anterior) - cuadrar para cuando sea diciembre
        tarifas['ipc_mes_anterior'] = ipcMesAnterior
        tarifas['ipc_mes_consultado'] = ipcMesConsulta
    ...
    def getIPC(self, mongodb, ano, periodo, mes):
    ...
    def getVariablesSUI(self, tarifa):
        sql = tarifas_sql
        return tarifa.db.engine.execute(text(sql), ANIO_ARG=tarifa.anio, PERIODO_ARG=tarifa.periodo, PERIODO_ARG_MENOS1=tarifa.periodo_menos1, EMPRESA_ARG=tarifa.empresa, MERCADO_ARG=tarifa.mercado, NTPROP_ARG=tarifa.ntprop).fetchall()
